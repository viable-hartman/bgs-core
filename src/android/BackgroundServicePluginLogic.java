	...
	protected class ServiceDetails {
		...
		private JSONObject createJSONResult(Boolean success, int errorCode, String errorMessage) {
			...
			if (this.mServiceConnected != null && this.mServiceConnected && this.isServiceRunning()) {
				...
				try { result.put("TimerEnabled", (Boolean)null); } catch (Exception ex) {Log.d(LOCALTAG, "Adding TimerEnabled to JSONObject failed", ex);};
				...
				try { result.put("TimerMilliseconds", 90000); } catch (Exception ex) {Log.d(LOCALTAG, "Adding TimerMilliseconds to JSONObject failed", ex);};
			}
